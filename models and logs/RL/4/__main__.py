...
def obs2response(model, obs):
    ...
    action = _postprocess_action(agent, logits.detach().numpy().flatten(), obs['action_mask'])
...
if __name__ == '__main__':
    model = CNNModel()
    data_dir = os.environ.get('MODEL_PATH', '/data/model_11673.pkl')
    ckpt = torch.load(data_dir, map_location=torch.device('cpu'))
    # Support both raw state_dict and {'model': state_dict, ...} RL checkpoint format
