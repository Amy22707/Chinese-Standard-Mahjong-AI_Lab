...
class MahjongGBEnv():
    ...
    def __init__(self, config):
        ...
        self.reward_scale       = config.get('reward_scale', 1.0)    # divide all rewards by this
        self.reward_gang        = config.get('reward_gang', 0.5)      # bonus per kong completion
        self.reward_peng        = config.get('reward_peng', 0.2)      # bonus per pong completion
        self.reward_chi         = config.get('reward_chi', 0.1)       # bonus per chow completion
        self.reward_tenpai      = config.get('reward_tenpai', 2.0)    # tenpai bonus at Huang
        self.reward_notenpai    = config.get('reward_notenpai', -2.0) # penalty for not tenpai at Huang
    ...
    def reset(self, prevalentWind = -1, tileWall = ''):
        ...
        self.reward = None
        self.done = False
        self._step_rewards = [0.0] * 4  # per-step reward accumulator
        # Init random seed
        if self.variety > 0:
    ...
    def step(self, action_dict):
        self._step_rewards = [0.0] * 4  # reset per-step reward at start of each step
        try:
            if self.state == 0:
                ...
                for j in range(1, 4):
                    ...
                    for j in range(1, 4):
                        ...
                        if t[i][0] == 'Chi' and not self.wallLast:
                            ...
                            if self.wallLast:
                                # Draw (流局): apply tenpai bonus/penalty.
                                # Skip the expensive _is_tenpai check when both rewards are 0.
                                huang_rewards = [0.0] * 4
                                if self.reward_tenpai != 0 or self.reward_notenpai != 0:
                                    for p in range(4):
                                        if self._is_tenpai(p):
                                            huang_rewards[p] = self.reward_tenpai
                                        else:
                                            huang_rewards[p] = self.reward_notenpai
                                self.obs = {i : self.agents[i].request2obs('Huang') for i in range(4)}
                                self.reward = huang_rewards
                                self.done = True
                            else:
                                # Next player
    ...
    def _reward(self):
        if self.reward:
            return {
                self.agent_names[k] : (self.reward[k] / self.reward_scale + self._step_rewards[k])
                for k in self.obs
            }
        return {self.agent_names[k] : self._step_rewards[k] for k in self.obs}
    ...
    def _kong(self, player, tile):
        ...
        self._step_rewards[player] += self.reward_gang
        self._draw(player)
    ...
    def _pung(self, player, tile):
        ...
        self._step_rewards[player] += self.reward_peng
    ...
    def _chow(self, player, tile):
        ...
        self._step_rewards[player] += self.reward_chi
    ...
    def _is_tenpai(self, player):
        '''Check if player's current hand is in tenpai (can win with at least one tile).

        Iterates over all 34 tile types and stops as soon as one winning tile is found.
        Tiles already exhausted (shownTiles == 4) are skipped to reduce calls.
        '''
        hand  = self.hands[player]
        packs = self.packs[player]
        tile_list = getattr(self.agentclz, 'TILE_LIST', None)
        ...
        for win_tile in tile_list:
            # Skip tiles that are completely exhausted – they cannot be drawn
            if self.shownTiles[win_tile] >= 4:
                continue
            try:
                fans = MahjongFanCalculator(
                    pack          = tuple(packs),
                    ...
                    flowerCount   = 0,
                    isSelfDrawn   = True,
                    is4thTile     = (self.shownTiles[win_tile] == 4),
                    isAboutKong   = False,
                    isWallLast    = True,
                    seatWind      = player,
                    prevalentWind = self.prevalentWind,
                ...
                fan_cnt = sum(fp * cnt for fp, cnt, _, _ in fans)
                if fan_cnt >= 8:
                    return True  # early exit – tenpai confirmed
            except Exception:
                pass
        return False