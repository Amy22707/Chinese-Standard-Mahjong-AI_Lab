...
class FeatureAgent(MahjongGBAgent):
    ...
    observation: 66*4*9
        ...
        shanten(1) + eff_tiles(1) + chiitoi(1) + flush_WTB(3)
        = 60 + 1 + 1 + 1 + 3 = 66 channels, each over 4*9 tile grid

    Matches the SL feature representation exactly for transfer learning compatibility.
    ...
    def __init__(self, seatWind):
        ...
        # Cache for expensive shanten-based features (recomputed only when hand/packs change)
        self._shanten_cache = None   # (hand_key, shanten, eff_tiles_arr, chiitoi, flush_arr)
    ...
    def request2obs(self, request):
        ...
        if t[2] == 'AnGang':
            ...
            if p == 0:
                ...
                self._hand_embedding_update()  # update HAND channel after removing 4 tiles
            else:
                self.isAboutKong = False
            return
        if t[2] == 'BuGang':
    ...
    def _hand_embedding_update(self):
        ...
        # Invalidate shanten cache whenever hand changes
    ...
    def _public_embedding_update(self):
        ...
        # Combines genbutsu, suji hints and exposed-meld suit pressure into 0..1 risk score.
        for k in range(3):
        ...
        # Shanten / EFF_TILES / CHIITOI / FLUSH – expensive features cached by hand key.
        # Only recomputed when hand changes (_hand_embedding_update invalidates the cache).
        hand_key = (tuple(sorted(self.hand)), tuple(sorted(str(p) for p in self.packs[0])))
        if self._shanten_cache is None or self._shanten_cache[0] != hand_key:
            self._shanten_cache = (hand_key,) + self._compute_shanten_features()
        _, shanten_val, eff_arr, chiitoi_val, flush_arr = self._shanten_cache

        self.obs[self.OFFSET_OBS['SHANTEN'], :] = shanten_val
        self.obs[self.OFFSET_OBS['EFF_TILES'], :] = eff_arr
        self.obs[self.OFFSET_OBS['CHIITOI'], :] = chiitoi_val
        self.obs[self.OFFSET_OBS['FLUSH']:self.OFFSET_OBS['FLUSH'] + 3, :] = flush_arr

    def _compute_shanten_features(self):
        '''Compute shanten-based features. Returns (shanten_val, eff_arr, chiitoi_val, flush_arr).
        Called at most once per hand-change event (cached in _shanten_cache).
        EFF_TILES uses precise MahjongShanten comparison – safe because this method
        is only invoked when the hand changes, not on every step.
        '''
        eff_arr   = np.zeros(36, dtype=np.float32)
        flush_arr = np.ones((3, 36), dtype=np.float32)

        # SHANTEN + EFF_TILES: precise calculation via MahjongShanten
        shanten_val = 0.5
        try:
            from MahjongGB import MahjongShanten
            base_shanten = MahjongShanten(hand=tuple(self.hand), pack=tuple(self.packs[0]))
            shanten_val = max(0, base_shanten) / 8.0
            if base_shanten >= 0:
                handCnt = defaultdict(int)
                for tile in self.hand:
                    handCnt[tile] += 1
                for tile in self.TILE_LIST[:34]:  # only suit/honour tiles
                    remaining = max(0, 4 - handCnt[tile] - self.shownTiles[tile])
                    if remaining == 0:
                        continue
                    new_sh = MahjongShanten(hand=tuple(self.hand) + (tile,), pack=tuple(self.packs[0]))
                    if new_sh < base_shanten:
                        eff_arr[self.OFFSET_TILE[tile]] = remaining / 4.0
        except Exception:
            pass

        # CHIITOI (no external call needed)
        handCnt = defaultdict(int)
        for tile in self.hand:
            handCnt[tile] += 1
        pairs = sum(1 for c in handCnt.values() if c >= 2)
        chiitoi_val = max(0, 6 - pairs) / 6.0

        # FLUSH shanten per suit W/T/B
        try:
            for s_idx, suit in enumerate('WTB'):
                ...
                flush_arr[s_idx, :] = max(0, 13 - len(suit_tiles)) / 13.0
        except Exception:
            pass

        return shanten_val, eff_arr, chiitoi_val, flush_arr
    ...
    def _estimate_discard_danger(self, player, tile):
        ...
        if tile in self.history[player]:
            return 0.0  # genbutsu: already discarded = safe
        ...
        if tile[0] in 'WTB':
            color = tile[0]
            num = int(tile[1])
            # Suji: if player discarded suji tile, middle tiles are safer
            suji_tiles = []
            if num <= 6:
                suji_tiles.append(color + str(num + 3))
            if num >= 4:
                suji_tiles.append(color + str(num - 3))
            for suji in suji_tiles:
                if suji in self.history[player]:
                    danger *= 0.6
            # Terminal/near-terminal tiles are generally safer
            if num in (1, 9):
                danger *= 0.7
            elif num in (2, 8):
                danger *= 0.85
        # Exposed meld pressure: if player has a meld in this suit, non-suit tiles safer
        for pack_type, pack_tile, offer in self.packs[player]:
            if pack_tile != 'CONCEALED' and pack_tile[0] != tile[0] and tile[0] in 'WTB':
                danger *= 0.9
