...
def _discard_sequence_tensors(agent):
    events = []
    for rel_player, hist in enumerate(agent.history):
        for order, tile in enumerate(hist):
...
def _postprocess_action(agent, logits, mask, aux = None):
    ...
    for tile, idx in FeatureAgent.OFFSET_TILE.items():
        ...
        if legal_mask[a]:
            ...
            if USE_RISK_HEAD and aux is not None and 'risk' in aux and idx < 34:
                danger = max(danger, float(1.0 / (1.0 + np.exp(-aux['risk'][idx]))))
    ...
    if should_gate and play_begin <= final_action < chi_begin:
        ...
        if USE_RISK_HEAD and aux is not None and 'risk' in aux:
            idx = final_action - play_begin
            final_danger = max(final_danger, float(1.0 / (1.0 + np.exp(-aux['risk'][idx]))))
        ...
        if final_danger >= danger_threshold:
            ...
            for tile, idx in FeatureAgent.OFFSET_TILE.items():
                ...
                if raw[a] < raw[final_action] - close_margin:
                    continue
                tile_danger = _discard_danger(agent, tile)
                if USE_RISK_HEAD and aux is not None and 'risk' in aux and idx < 34:
                    tile_danger = max(tile_danger, float(1.0 / (1.0 + np.exp(-aux['risk'][idx]))))
