...
def _postprocess_action(agent, logits, mask, aux = None):
    ...
    safe_bonus = 0.55 if shanten >= 2 and not ctx['high_value'] else 0.30

    # Push far or low-value hands toward safety, while keeping tenpai/high-value hands alive.
    for tile, idx in FeatureAgent.OFFSET_TILE.items():
        ...
        if legal_mask[a]:
            ...
            if aux is not None and 'discard_rank' in aux and idx < 34:
                adjusted[a] += 0.4 * (float(aux['discard_rank'][idx]) - 0.5)
