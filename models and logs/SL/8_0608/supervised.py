...
def move_batch(batch, device):
    ...
    }, act.long().to(device, non_blocking = non_blocking), wt.float().to(device, non_blocking = non_blocking)
...
if __name__ == '__main__':
    ...
    for e in range(start_epoch, args.epochs):
        ...
        for i, d in enumerate(loader):
            input_dict, target, wt = move_batch(d, device)
            logits, type_logits = model(input_dict, return_type_logits = True)
            ...
            loss = policy_loss + args.type_loss_weight * type_loss
            if i % 128 == 0:
        ...
        for i, d in enumerate(vloader):
            input_dict, target, _ = move_batch(d, device)
            with torch.no_grad():
