...
def _postprocess_action(agent, logits, mask):
    ...
    # shanten_factor = 1.0 when far from tenpai (full penalty), 0.0 when tenpai (no penalty).
    shanten_factor = 1.0
    ...
    # Also check seven-pairs tenpai for bonus on keeping pairs.
    if hasattr(agent, 'hand') and hasattr(agent, 'packs'):
        try:
            from MahjongGB import MahjongShanten
            shanten = MahjongShanten(hand = tuple(agent.hand), pack = tuple(agent.packs[0]))
                ...
                for p in range(1, 4):
                    for t in set(agent.history[p]):
                        if t in FeatureAgent.OFFSET_TILE:
                            ...
                            if legal_mask[a]:
                                adjusted[a] -= 0.15  # cancel half of the +0.3 genbutsu bonus
                # Suppress Pass so bot actively pursues the win rather than waiting passively.
                adjusted[FeatureAgent.OFFSET_ACT['Pass']] -= 1.0

            # Seven-pairs awareness: count pairs in hand.
            from collections import Counter
            hand_cnt = Counter(agent.hand)
            pairs = sum(1 for c in hand_cnt.values() if c >= 2)
            chiitoi_shanten = 6 - pairs  # need 7 pairs for chiitoi
            # If chiitoi is closer or tied with regular shanten, penalise discarding a pair tile.
            if chiitoi_shanten <= shanten and chiitoi_shanten <= 2:
                for tile, cnt in hand_cnt.items():
                    if cnt >= 2 and tile in FeatureAgent.OFFSET_TILE:
                        a = play_begin + FeatureAgent.OFFSET_TILE[tile]
                        if legal_mask[a]:
                            adjusted[a] -= 0.5  # discourage breaking pairs
        except Exception:
            pass
...
if __name__ == '__main__':
    model = CNNModel()
    data_dir = os.environ.get('MODEL_PATH', '/data/best3.pkl')
