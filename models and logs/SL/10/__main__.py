...
USE_AUX_RANK = os.environ.get('USE_AUX_RANK', '1') == '1'
...
def _postprocess_action(agent, logits, mask, aux = None):
    ...
    safe_bonus = 0.35 if shanten >= 2 and not ctx['high_value'] else 0.20
    ...
    rerank_margin = 0.85 if shanten <= 1 or ctx['high_value'] else 1.25

    # Mild risk control: only rerank discard candidates already close to the model's choice.
    for tile, idx in FeatureAgent.OFFSET_TILE.items():
        ...
        if legal_mask[a]:
            ...
            if shanten >= 2 and not ctx['high_value'] and danger >= 0.65:
                adjusted[a] -= 0.20
            if danger <= 0.05:
                adjusted[a] += safe_bonus
            if USE_AUX_RANK and aux is not None and 'discard_rank' in aux and idx < 34:
                adjusted[a] += 0.15 * (float(aux['discard_rank'][idx]) - 0.5)
    ...
    adjusted[chi_begin:peng_begin] -= 0.25
    adjusted[peng_begin:gang_begin] -= 0.20
    adjusted[gang_begin:angang_begin] -= 0.15
    adjusted[bugang_begin:] -= 0.15
