...
class FeatureAgent(MahjongGBAgent):
    ...
    observation: 70*4*9
        ...
        shanten(1) + effective_tiles(1) + chiitoi(1) + flush_WTB(3) +
        public score context(4, optional, zero if unavailable)
        = 70 channels, each over 4*9 tile grid
    action_mask: 235
    ...
    OFFSET_OBS = {
        ...
        'HAND'           : 2,   # 4 ch: my hand count encoding (ch k = has at least k+1 copies)
        ...
        'DANGER'         : 53,  # 3 ch: opponent seats 1/2/3; estimated discard risk
        'DISCARD_POS'    : 56,  # 4 ch: per player; normalised recency of each tile in discard seq
        'SHANTEN'        : 60,  # 1 ch: normalised shanten number broadcast over all slots (0=tenpai)
        'EFF_TILES'      : 61,  # 1 ch: per-tile remaining count if it is an effective tile, else 0
        'CHIITOI'        : 62,  # 1 ch: normalised seven-pairs shanten broadcast
        'FLUSH'          : 63,  # 3 ch: normalised flush (chinitsu) shanten per suit W/T/B broadcast
        'SCORE_CTX'      : 66,  # 4 ch: observable score/rank context, zero when not supplied
    }
    OFFSET_ACT = {
        'Pass' : 0,
        'Hu' : 1,
    ...
    def set_public_scores(self, scores):
        '''Set observable score context if the platform provides standings before the hand.

        This must not be called with terminal Score lines during preprocessing, because that
        would leak future information unavailable during actual play.
        '''
        ...
        try:
            ...
            return
        my_score = scores[self.seatWind]
        rank = 1 + sum(s > my_score for s in scores)
        leader_gap = max(scores) - my_score
        last_gap = my_score - min(scores)
        ch = self.OFFSET_OBS['SCORE_CTX']
        self.obs[ch : ch + 4, :] = 0
        self.obs[ch, :] = max(-1.0, min(1.0, my_score / 128.0))
        self.obs[ch + 1, :] = (rank - 1) / 3.0
        self.obs[ch + 2, :] = max(0.0, min(1.0, leader_gap / 128.0))
        self.obs[ch + 3, :] = max(0.0, min(1.0, last_gap / 128.0))
    ...
    def _public_embedding_update(self):
        # Reset derived public channels, but keep optional score context if it was supplied.
        self.obs[self.OFFSET_OBS['SHOWN'] : self.OFFSET_OBS['SCORE_CTX']] = 0
        ...
        # Combines genbutsu (discarded by that player = safe), visible-wall pressure,
        # suji hints and exposed-meld suit pressure into a 0..1 risk score.
        for k in range(3):
            ch = self.OFFSET_OBS['DANGER'] + k
            player = k + 1
            for tile in self.TILE_LIST:
                self.obs[ch, self.OFFSET_TILE[tile]] = self._estimate_discard_danger(player, tile)
        ...
        # Shanten number: broadcast normalised value over all 36 slots.
        # shanten -1 (already won) -> 0.0, shanten 0 (tenpai) -> 0.0,
        # shanten k -> k/8  (hand can be at most 8-away from tenpai)
        try:
            from MahjongGB import MahjongShanten
            shanten = MahjongShanten(hand = tuple(self.hand), pack = tuple(self.packs[0]))
            self.obs[self.OFFSET_OBS['SHANTEN'], :] = max(0, shanten) / 8.0
        except Exception:
            self.obs[self.OFFSET_OBS['SHANTEN'], :] = 0.5  # unknown: use mid-range

        # Effective tiles: per-tile remaining count (normalised) if that tile reduces shanten.
        # Value = remaining_copies / 4  for effective tiles, 0 otherwise.
        self.obs[self.OFFSET_OBS['EFF_TILES'], :] = 0
        try:
            handCnt = defaultdict(int)
            for tile in self.hand:
                handCnt[tile] += 1
            base_shanten = MahjongShanten(hand = tuple(self.hand), pack = tuple(self.packs[0]))
            if base_shanten >= 0:
                for tile in self.TILE_LIST[:34]:  # only 34 suit/honour tiles, no flowers
                    remaining = max(0, 4 - handCnt[tile] - self.shownTiles[tile])
                    if remaining == 0:
                        continue
                    new_hand = tuple(self.hand) + (tile,)
                    new_shanten = MahjongShanten(hand = new_hand, pack = tuple(self.packs[0]))
                    if new_shanten < base_shanten:
                        self.obs[self.OFFSET_OBS['EFF_TILES'], self.OFFSET_TILE[tile]] = remaining / 4.0
        except Exception:
            pass

        # Seven-pairs shanten: broadcast normalised value.
        # Counts distinct pairs; shanten = 6 - pairs (need 7 pairs, start with 0).
        try:
            handCnt = defaultdict(int)
            for tile in self.hand:
                handCnt[tile] += 1
            pairs = sum(1 for c in handCnt.values() if c >= 2)
            chiitoi_shanten = max(0, 6 - pairs)
            self.obs[self.OFFSET_OBS['CHIITOI'], :] = chiitoi_shanten / 6.0
        except Exception:
            self.obs[self.OFFSET_OBS['CHIITOI'], :] = 1.0

        # Flush (chinitsu) shanten per suit W/T/B: how far from a single-suit hand.
        # For each suit, count tiles of that suit in hand; shanten ~ 13 - count (rough).
        try:
            for s_idx, suit in enumerate('WTB'):
                suit_tiles = [t for t in self.hand if t[0] == suit]
                # also count suit tiles already in own packs
                for pack_type, tile, offer in self.packs[0]:
                    if tile != 'CONCEALED' and tile[0] == suit:
                        if pack_type == 'CHI':
                            suit_tiles.extend([tile[0] + str(int(tile[1]) + i) for i in range(-1, 2)])
                        elif pack_type == 'PENG':
                            suit_tiles.extend([tile] * 3)
                        elif pack_type == 'GANG':
                            suit_tiles.extend([tile] * 4)
                flush_shanten = max(0, 13 - len(suit_tiles)) / 13.0
                self.obs[self.OFFSET_OBS['FLUSH'] + s_idx, :] = flush_shanten
        except Exception:
            self.obs[self.OFFSET_OBS['FLUSH'] : self.OFFSET_OBS['FLUSH'] + 3, :] = 1.0
    ...
    def _estimate_discard_danger(self, player, tile):
        if tile not in self.OFFSET_TILE:
            return 0.0
        if tile in self.history[player]:
            return 0.0
        visible = self.shownTiles[tile] + self.hand.count(tile)
        remaining = max(0, 4 - visible)
        if remaining == 0:
            return 0.0
        danger = 0.25 + 0.15 * remaining
        if tile[0] in 'WTB':
            color = tile[0]
            num = int(tile[1])
            discarded = set(self.history[player])
            if num <= 3 and color + str(num + 3) in discarded:
                danger -= 0.18
            if 4 <= num <= 6 and (color + str(num - 3) in discarded or color + str(num + 3) in discarded):
                danger -= 0.18
            if num >= 7 and color + str(num - 3) in discarded:
                danger -= 0.18
            if num in (1, 9):
                danger -= 0.05
            same_suit_packs = 0
            total_suit_packs = 0
            for pack_type, pack_tile, offer in self.packs[player]:
                if pack_tile != 'CONCEALED' and pack_tile[0] in 'WTB':
                    total_suit_packs += 1
                    if pack_tile[0] == color:
                        same_suit_packs += 1
            if same_suit_packs >= 2:
                danger += 0.18
            elif same_suit_packs == 1 and total_suit_packs == 1:
                danger += 0.06
        else:
            danger += 0.08
            if visible >= 2:
