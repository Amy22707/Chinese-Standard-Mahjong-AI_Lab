...
def _discard_progress_score(agent, discard_tile, base_shanten):
    if not hasattr(agent, 'hand') or discard_tile not in agent.hand:
        return 0.0
    try:
        from MahjongGB import MahjongShanten

        hand_after = list(agent.hand)
        hand_after.remove(discard_tile)
        after_shanten = MahjongShanten(hand = tuple(hand_after), pack = tuple(agent.packs[0]))
        score = 0.0
        if after_shanten > base_shanten:
            score -= 0.80
        elif after_shanten < base_shanten:
            score += 0.45

        effective_left = 0
        hand_cnt = {}
        for tile in hand_after:
            hand_cnt[tile] = hand_cnt.get(tile, 0) + 1
        for tile in FeatureAgent.TILE_LIST[:34]:
            remaining = max(0, 4 - hand_cnt.get(tile, 0) - agent.shownTiles[tile])
            if remaining == 0:
                continue
            new_shanten = MahjongShanten(hand = tuple(hand_after + [tile]), pack = tuple(agent.packs[0]))
            if new_shanten < after_shanten:
                effective_left += remaining
        score += min(0.45, effective_left / 40.0)
        return score
    except Exception:
        return 0.0
...
def _postprocess_action(agent, logits, mask, aux = None):
    ...
    shanten = ctx['shanten']
    defense_scale = _defense_scale(shanten, ctx['high_value'])
    if ctx['high_value'] and shanten >= 2:
        defense_scale = max(defense_scale, 0.75)
    safe_bonus = 0.20 if shanten >= 2 else 0.08
    legal_logits = raw.copy()
    legal_logits[~legal_mask] = -np.inf
    raw_best = int(legal_logits.argmax())

    play_mask = legal_mask[play_begin:chi_begin]
    best_play_logit = -np.inf
    if play_mask.any():
        best_play_logit = float(raw[play_begin:chi_begin][play_mask].max())
    rerank_margin = 0.45 if shanten <= 1 else 0.75

    # Rerank close discard candidates by safety and hand progress, without forcing a fold.
    for tile, idx in FeatureAgent.OFFSET_TILE.items():
        a = play_begin + idx
        if legal_mask[a]:
            if raw[a] < best_play_logit - rerank_margin:
                continue
            danger = _discard_danger(agent, tile)
            adjusted[a] -= defense_scale * danger
            if shanten >= 2 and danger >= 0.75:
                adjusted[a] -= 0.10
            if danger <= 0.05:
                adjusted[a] += safe_bonus
            adjusted[a] += _discard_progress_score(agent, tile, shanten)
    ...
    # Melds reduce defensive flexibility. Be conservative for far/low-value hands,
    # but keep near-tenpai or high-value hands free to push.
    if shanten >= 2 and not ctx['high_value']:
        chi_penalty, peng_penalty, gang_penalty, bugang_penalty = 0.45, 0.35, 0.30, 0.25
    elif shanten == 1 or (ctx['high_value'] and shanten <= 2):
        chi_penalty, peng_penalty, gang_penalty, bugang_penalty = 0.25, 0.20, 0.15, 0.15
    else:
        chi_penalty, peng_penalty, gang_penalty, bugang_penalty = 0.15, 0.12, 0.10, 0.10
    ...
    # In light mode, never replace a clearly preferred discard with a heuristic-safe discard.
    if play_begin <= raw_best < chi_begin and play_begin <= final_action < chi_begin:
        if raw[raw_best] >= raw[final_action] + 0.45:
            return raw_best
