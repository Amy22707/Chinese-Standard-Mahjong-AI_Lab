...
def _postprocess_action(agent, logits, mask, aux = None):
    ...
    safe_bonus = 0.20 if shanten >= 2 and not ctx['high_value'] else 0.08
    ...
    rerank_margin = 0.35 if shanten <= 1 or ctx['high_value'] else 0.60

    # Very light risk control: only rerank discard candidates already close to the model's choice.
    for tile, idx in FeatureAgent.OFFSET_TILE.items():
        ...
        if legal_mask[a]:
            ...
            if shanten >= 2 and not ctx['high_value'] and danger >= 0.75:
                adjusted[a] -= 0.10
            if danger <= 0.05:
