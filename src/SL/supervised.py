    ...
    for e in range(start_epoch, args.epochs):
        ...
        for i, d in enumerate(loader):
            ...
            risk_loss = F.binary_cross_entropy_with_logits(
                aux_pred['risk'][play_legal],
                aux_target['risk'].float()[play_legal]
            ) if play_legal.any() else logits.new_tensor(0.0)
            ...
            if i % 128 == 0:
                print('Iteration %d/%d'%(i, len(trainDataset) // args.batch_size + 1),
                      ...
                      'risk_loss', risk_loss.item(), 'fan_route_loss', fan_route_loss.item(),
                      flush = True)
