...
\subsection{最终版本与代码评估补充}

在多轮实现和自对弈调参后，当前代码已经不只是一个基础 SL 分类器，而是一个包含
特征工程、分解策略网络、辅助监督、推理后处理和可选 RL 微调框架的完整麻将 Bot。
主要版本演进可以概括如下：

\begin{enumerate}
  \item \textbf{基础 SL 版本}：使用 Botzone 日志构造动作分类数据，按 235 维动作空间直接监督学习，
  能完成合法动作选择，但对向听数、防守风险和高番路线缺乏显式建模。
  \item \textbf{特征增强版本}：将观测扩展到 66 通道，加入手牌、副露、弃牌、剩余牌、牌墙、
  现物危险度、弃牌位置、向听数、有效牌、七对子和清一色/混一色路线等公开信息。
  \item \textbf{ResNet 分头版本}：使用残差卷积主干，并把动作拆成动作类型头和具体牌/吃法子头，
  缓解 235 维动作空间中不同动作类型样本不均衡的问题。
  \item \textbf{训练数据优化版本}：加入赢家过滤、番数加权、Weighted Sampler、花色置换增强、
  延迟 LRU 数据缓存、多进程预处理和断点续训，使模型更偏向学习高质量赢家样本。
  \item \textbf{推理后处理版本}：在模型 logit 上叠加规则修正，包括胡牌优先、听牌时降低 Pass、
  现物奖励、危险牌惩罚、七对子保对子、吃碰杠惩罚和后期危险牌折返门控。
  \item \textbf{最终重训版本}：进一步扩展到 70 通道，新增可观测分数/名次差上下文；
  网络加入弃牌序列 GRU、风险头、番型路线头、胜率/番数/向听/弃牌排序辅助头；
  训练目标改为条件式分层损失，即动作类型、子动作和完整策略共同监督。
\end{enumerate}

当前 \texttt{src/SL} 的功能分工如下。\texttt{feature.py} 负责把国标麻将局面编码为
70 通道张量，同时只使用比赛时可见的信息；四家分数通道通过 \texttt{set\_public\_scores}
显式传入，避免训练时使用上帝视角手牌造成信息泄漏。\texttt{preprocess.py} 将牌谱转为
\texttt{npz} 样本，记录全局弃牌序列、赢家权重、风险标签和番型路线标签。\texttt{dataset.py}
提供懒加载缓存和花色置换增强。\texttt{model.py} 采用 ResNet+GRU 融合结构，并输出分解动作
logit 与多任务辅助头。\texttt{supervised.py} 使用 AdamW、余弦退火、加权损失和辅助损失训练。
\texttt{\_\_main\_\_.py} 则是 Botzone 交互入口，在模型输出后做轻量的攻守平衡后处理。

当前 \texttt{src/RL} 实现了 PPO 自博弈训练框架。\texttt{actor.py} 并行采样对局，
\texttt{model\_pool.py} 维护历史模型池，使对手有一定多样性；\texttt{learner.py} 使用
GAE、PPO clipped objective、价值函数、熵正则和 SL teacher KL 约束更新策略；
\texttt{env.py} 使用国标麻将环境并加入流局听牌/未听牌奖励。该框架具备从 SL checkpoint
warm start 的能力，但目前 RL 网络仍主要是策略-价值结构，没有继承 SL 最终版中的 GRU 序列、
风险头和番型路线辅助任务，因此在短训练预算下不如 SL 稳定。

\subsection{RL 不如 SL 的原因}

结合文献综述可以看到，强麻将 Bot 往往先依赖大规模、高质量牌谱做 SL，再用非常长时间的
自博弈微调。当前 RL 比不过 SL 的主要原因不是框架方向错误，而是训练预算和样本效率不匹配。
国标麻将是四人不完全信息博弈，奖励稀疏且延迟，一次点炮、流局或自摸的得分要回溯到几十步
之前的取舍，PPO 在这种场景下需要海量自博弈样本才能形成稳定改进。相比之下，SL 直接从强
Bot/赢家牌谱中学习，起点已经包含大量有效先验。

其次，当前 RL 的 reward shaping 只能粗略表达胡牌得分、流局听牌和未听牌惩罚，难以精确描述
“什么时候应该做大牌、什么时候应该收手防守”。如果 shaping 过强，模型可能学到保听牌或盲目
进攻等局部最优行为；如果 shaping 过弱，又会回到稀疏奖励导致学习极慢。多智能体自博弈还会
带来非平稳性：模型池中的对手不断变化，旧采样数据与当前策略分布不完全一致，PPO ratio 和
价值估计的方差都会上升。由于算力和时间限制，本项目无法进行文献中顶尖系统那种数量级的
长周期自博弈，因此 RL 尚未充分超过 SL。

最后，SL 最终版已经加入了很多对比赛有效的工程先验，如风险估计、现物、防守门控、番型路线
和弃牌序列；而 RL 部分尚未完全复用这些结构。更合理的后续路线是：先用当前 SL 作为强 teacher，
冻结一部分特征和序列编码器，使用较小学习率和 KL 约束做长期微调；同时把点炮惩罚、放铳对象、
局面名次差和对手威胁度显式纳入 reward 与 value head。

\subsection{IJCAI 积分赛结果分析}

最终 Bot 在 IJCAI 积分赛中取得第 12 名，说明该方案已经具备较强的实战竞争力。它的优势主要
来自三个方面：第一，特征工程充分利用了国标麻将公开信息，使模型不仅看见手牌，还能感知
牌墙进度、弃牌结构、向听进度和基本危险度；第二，分解动作头与辅助监督降低了动作空间学习
难度，提升了吃碰杠和打牌之间的校准能力；第三，推理后处理让模型在胡牌优先、听牌进攻、
现物防守和大牌路线之间形成了可控折中。

不足也比较清晰。当前 Bot 仍然依赖人工调参的后处理，面对强对手时点炮风险估计还不够细；
面对弱对手时有时不能充分扩大进攻收益，面对强对手时又缺乏更精确的对手手牌概率推断。
此外，比赛中分数、名次差和局数阶段会影响策略，但当前分数特征只是预留了可观测上下文，
尚未在大规模训练中充分学习。下一步最有价值的改进，是使用更高质量和更多样的牌谱重训
风险/番型辅助头，加入真实名次差策略，并在 SL teacher 约束下做长期 RL 微调。
...
\begin{tikzpicture}[
  >=Latex,
  flowstep/.style={rectangle, draw=black!70, rounded corners=3pt,
    fill=yellow!10, minimum width=11cm, minimum height=0.65cm,
    text centered, font=\small},
  node distance=0.38cm
]
\node[flowstep] (raw)  {原始模型 logit（235 维）};
\node[flowstep, below=of raw]  (s1) {① 向听感知危险惩罚（式~\ref{eq:danger_pen}）：$a_{\text{play}} \mathrel{-}= 2.0 \cdot d(t) \cdot \min(1, \text{shanten}/3)$};
\node[flowstep, below=of s1]   (s2) {② 现物奖励（式~\ref{eq:genbutsu}）：安全牌 $+0.3$};
\node[flowstep, below=of s2]   (s3) {③ 听牌进攻：$\text{shanten}=0$ 时 Pass $-1.0$};
\node[flowstep, below=of s3]   (s4) {④ 七对子保护：对子牌 $-0.5$（七对向听 $\leq 2$ 时）};
\node[flowstep, below=of s4]   (s5) {⑤ 轻碰/杠惩罚：吃$-0.45$，碰$-0.35$，杠$-0.30$，补杠$-0.25$};
\node[flowstep, below=of s5, fill=green!10]   (end) {非法动作填 $-100.0$，取 argmax 输出动作};
